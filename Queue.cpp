...
class Queue
{
    private:
        ...
        int deqfr;		 //the front element which returns after dequeue() is called
    public:
        Queue();
        ~Queue();
        void enqueue(T);
...
int main()
{
    ...
    srand((unsigned int) time(NULL));    //use system time as a seed for rand()
...
void display()
{
    ...
    {
          cout<<spaces<<"║";
          if(i<road[TOP].size)    cout<<road[TOP].array[road[TOP].size-1-(((road[TOP].front+i))%road[TOP].capacity)].getColor()<<setw(4)<<" ▒▒ "<<"\033[0m";
          else                    cout<<"    ";
    ...
    {
        cout<<"║";
        if(i<road[LEFT].size)    cout<<road[LEFT].array[road[LEFT].size-1-((road[LEFT].front+i))%road[LEFT].capacity].getColor()<<setw(4)<<" ▒▒ "<<"\033[0m";
        else                    cout<<"    ";
    }
    cout<<"║";
    ...
    {
        cout<<"║";
        if(i<road[RIGHT].size)    cout<<road[RIGHT].array[road[RIGHT].size-1-(road[RIGHT].front+i)%road[RIGHT].capacity].getColor()<<setw(4)<<" ▒▒ "<<"\033[0m";
        else                    cout<<"    ";
    }
    cout<<"║"<<endl<<"╚════";
    ...
    for(int i=0; i<QUEUE_SIZE; i++)
    {
        ...
        if(i<road[BOTTOM].size)    cout<<road[BOTTOM].array[road[BOTTOM].size-1-((road[BOTTOM].front+i))%road[BOTTOM].capacity].getColor()<<setw(4)<<" ▒▒ "<<"\033[0m"<<"║"<<endl;
        else                    cout<<"    ║"<<endl;
    }
...
//===========================================================================================
template<typename T> Queue<T>::Queue() : array(new T[QUEUE_SIZE]), size(0), capacity(QUEUE_SIZE), rear(0), front(0) {}
...
//===========================================
template<typename T> bool Queue<T>::isEmpty() {return (size == 0); }

//===========================================
template<typename T> bool Queue<T>::isFull() {return (size == capacity); }
