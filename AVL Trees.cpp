...
int AVL::balanceFactor(Node *ptr)
{
    ...
    //Return the difference in height from the left side and the height from the right side of the tree.
...
void AVL::rotateRight(Node *y)
{
    ...
    //Re-assigning links: z becomes the parent of T2
    if(T2 == NULL) {
        z->left = NULL;
    }
    ...
    //If z is not the root, re-assign the parent links.
    if(z != root) {
        if(z == z->parent->left) {
            y->parent = z->parent;
            z->parent->left = y;
    ...
    //If z is the root, then y can simply be the root.
    else if(z == root) {
        root = y;
    }
    
    //Re-assigning links: y becomes the parent of z
    y->right = z;
    z->parent = y;
}
...
{
    //Find the balance factor for ptr
    int balance = balanceFactor(ptr);
    
    //If the balance factor is < -1, it implies it is Right Heavy:
    if(balance < -1) {
        //Find balance factor on the right side
        balance = balanceFactor(ptr->right);
        
        //If the new balance factor is > 0, it implies it is Right-Left Heavy
        if(balance > 0) {
            rotateRight(ptr->right->left);
            rotateLeft(ptr->right);
        }
        
        //Else it is Right-Right Heavy
        else {
            rotateLeft(ptr->right);
        }
    }
    
    //If the balance factor is > 1, it implies it is Left Heavy:
    else if(balance > 1) {
        //Find balance factor on the left side
        balance = balanceFactor(ptr->left);
        
        //If the new balance factor is > 0, it implies it is Left-Left Heavy
        if(balance > 0) {
            rotateRight(ptr->left);
        }
        
        //Else it is Left-Right Heavy
        else {
            rotateLeft(ptr->left->right);
            rotateRight(ptr->left);
        }
    }
    
    //Default case, if it is already balanced, simply return.
    else
        return;
}
